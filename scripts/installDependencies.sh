...
if [ -z "$CARES" ]; then
  cd ~/temp/grpc/third_party/cares/cares &&
    CXX=g++-9 CC=gcc-9 cmake -DCMAKE_BUILD_TYPE=Release &&
...
# 2. install protobuf
PROTOBUF=$(protoc --version | grep "3.8")
if [ -z "$PROTOBUF" ]; then
  cd ~/temp/grpc/third_party/protobuf/cmake &&
    ...
    CXX=g++-9 CC=gcc-9 cmake -Dprotobuf_BUILD_TESTS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_POSITION_INDEPENDENT_CODE=ON .. &&
    make && sudo make install && sudo make clean && sudo ldconfig
  checkLastSuccess "install protobuf fails"
else
  echo "protobuf v3.8 has been installed, skip"
fi
# 3. install grpc
# install libssl-dev to skip installing boringssl
GRPC=$(grep "1.26" /usr/local/lib/cmake/grpc/gRPCConfigVersion.cmake)
if [ -z "$GRPC" ]; then
  cd ~/temp/grpc &&
    ...
    CXX=g++-9 CC=gcc-9 cmake -DgRPC_BUILD_TESTS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="-Wno-error=shadow" &&
    make && sudo make install && sudo make clean && sudo ldconfig
  checkLastSuccess "install grpc fails"
else
  echo "gRPC v1.26 has been installed, skip"
fi
# install rocksdb
