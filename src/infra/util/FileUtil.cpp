 ...
 */
std::string FileUtil::currentWorkingDir() {
 ...
 */
std::string FileUtil::getFileContent(const std::string &fileName) {
 ...
 */
void FileUtil::setFileContent(const std::string &fileName,
 ...
 */
void FileUtil::setFileContentWithSync(const std::string &fileName,
                                   const std::string &content) {
 ...
 */
std::vector<std::string> FileUtil::listFiles(const std::string &dir_) {
  namespace fs = boost::filesystem;
 ...
 */
std::vector<std::string> FileUtil::listDirs(const std::string &dir_) {
  namespace fs = boost::filesystem;
 ...
 */
int64_t FileUtil::getFileSize(int fd) {
 ...
 */
void FileUtil::setFileSize(int fd, uint64_t fileSize) {
 ...
 */
void FileUtil::syncAt(void *rawPtr, uint64_t length) {
 ...
 */
void FileUtil::writeStrToFile(std::ofstream &ofs, const std::string &content) {
  writeUint64ToFile(ofs, content.length());
  ofs.write(content.c_str(), content.length());
  // TODO(huyumars@gmail.com): Enhance file read/write error handling
  checkFileState(ofs);
}
 ...
 * @returns string
 */
std::string FileUtil::readStrFromFile(std::ifstream &ifs) {
 ...
 */
void FileUtil::writeUint64ToFile(std::ofstream &ofs, uint64_t content) {
  auto size = content;
 ...
 */
uint64_t FileUtil::readUint64FromFile(std::ifstream &ifs) {
  uint64_t size;
...
void FileUtil::checkFileState(std::ofstream &ofs) {
  if (!ofs) {
